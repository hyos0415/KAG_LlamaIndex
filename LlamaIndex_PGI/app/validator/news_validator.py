...
class ClaudeNewsValidator:
    ...
    def validate_article(self, article_text: str):
        ...
        # 검증 전용 프롬프트 (객관적 점수 로직 포함)
        validation_prompt_str = (
            "당신은 '뉴스 미디어 팩트체커'입니다. 아래 [사용자 기사]를 [지식 그래프 정보]와 대조하여 "
            "사실성(Factuality)과 독창성(Originality)을 정밀 검증하세요.\n\n"
            
            "### [점수 계산 로직]\n"
            "1. 사실성 (Factuality) = (그래프 매칭 엔티티 수 / 기사 내 전체 엔티티 수) * 100\n"
             "   - 장소, 날짜, 인물 등 핵심 팩트가 지식 그래프와 다를 경우 매칭 엔티티에서 제외합니다.\n"
            "2. 독창성 (Originality) = 30 + (Multi-hop 경로 깊이 * 20) + (고유 관계 수 * 15)\n"
            "   - 지식 그래프의 정보를 얼마나 창의적으로 연결하여 분석했는지 측정합니다 (최대 100점).\n\n"
            
            "### [리포트 형식]\n"
            "1. 검증 요약: [전체적인 신뢰도 평가]\n"
            "2. 사실 상충 항목 (Conflicts):\n"
            "   - [기사 내용] vs [지식 그래프 팩트] (구체적 상충 원인 기술)\n"
            "3. 객관적 점수 산출:\n"
            "   - 사실성 점수: [계산식] = [결과]/100\n"
            "   - 독창성 점수: [계산식] = [결과]/100\n\n"
