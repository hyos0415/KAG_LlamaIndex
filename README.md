...
*   **RAG**: 하이브리드 검색(BM25+벡터) 및 RRF 알고리즘 기반 검색 파이프라인 구축
*   **KAG**: 지식 그래프 기반 트리플렛 대조를 통한 논리적 사실 검증 — 탐지 구조 구현. 제어 효과는 미측정
...
- **RRF (Reciprocal Rank Fusion)**: 순수 알고리즘 구현(`Score = Σ 1/(rank + 60)`)으로 두 검색 결과를 결합합니다.
...
검색 단계에 대해 `RAGAS`로 측정했다. 파이프라인 분산 층 5개(수집·색인·검색·그래프·리포트) 중 검색 층만 측정됐다.

*   **RAG**: PoC 데이터셋 기준 Context Precision 1.0. 단 당시 색인된 문서는 5건이었으며(수집 40건 중), 표본이 작아 천장 효과 가능성이 높다. 개선 측정에는 기여하지 않는 값이다. 상세: `docs/design-review.md`
*   **KAG**: 지식 그래프 기반 트리플렛 대조 — 탐지 구조 구현. 제어 효과는 미측정
