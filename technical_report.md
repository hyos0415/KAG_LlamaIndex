...
| **Factuality** | LLM 기반 검증 가능한 사실(Triplets) 밀도 평가 (0~100) | 기확보된 지식(RAG)과의 정합성 및 논리적 타당성 측정 |
| **Depth** | `(2-hop 이상 경로 수) * 5`, Max 100 | 다단계 전이 관계의 복잡성과 깊이 측정 |
| **Originality** | 지식 베이스 기여도 및 신규 정보 비중 평가 | 기존 DB에 없는 새로운 사실(New Info)의 비중 및 가치 측정 |
